...
async def test_publish_command():
    ...
    dummy_cmd_queue = RabbitQueue(
        "dummy_cmd_queue", routing_key="command.ionode.inbound.fetch"
    )
    ...
    async with TestRabbitBroker(broker) as test_broker:
        ...
        await bus.publish_command(
            routing_key="command.ionode.inbound.fetch", command=command
        )
...
async def test_publish_event():
    ...
    dummy_evt_queue = RabbitQueue(
        "dummy_evt_queue", routing_key="event.resource.created"
    )
