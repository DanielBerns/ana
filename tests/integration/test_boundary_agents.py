        ...
        async with TestApp(app):
            command = ExecuteIONodeCommand(
                header=MessageHeader(
                    correlation_id="corr-999", source_component="test"
                ),
            ...
            assert (
                fetched_bytes
                == b'{"status": "success", "data": "Sample external data"}'
            )
    ...
    async with TestRabbitBroker(broker) as test_broker:
        ...
        async with TestApp(app):
            ...
            with pytest.raises(RejectMessage):
                ...
                await handle_inbound_command(
                    command, repository=temp_repository, bus=bus
                )
