...
class ProxyClient:
    ...
    async def _ensure_auth(self) -> None:
        ...
        try:
            ...
            raise ExpectedDomainException(
                f"Failed to authenticate with Ana Proxy: {e.response.status_code}"
            )
    ...
    async def fetch_pending_tasks_action(
        ...
        try:
            ...
            logger.debug(
                "Tasks fetched successfully", bytes_received=len(response.content)
            )
            ...
            if e.response.status_code == 401:
                ...
                logger.error(
                    "Failed to fetch tasks", status_code=e.response.status_code
                )
            raise ExpectedDomainException(
                f"Failed to fetch tasks: {e.response.status_code}"
            )

    async def upload_report_stream_action(
        self, parameters: dict[str, Any]
    ) -> tuple[bytes, str]:
        """
        ...
        try:
            response = await self.client.post(
                "/api/v1/reports", content=file_stream, headers=headers
            )
            response.raise_for_status()

            logger.info(
                "Report streamed successfully",
            ...
            raise ExpectedDomainException(
                f"Report upload failed: {e.response.status_code}"
            )
